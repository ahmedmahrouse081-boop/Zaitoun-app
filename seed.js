...
db.exec('DELETE FROM order_item_additions');
db.exec('DELETE FROM order_items');
db.exec('DELETE FROM orders');
db.exec('DELETE FROM item_additions');
db.exec('DELETE FROM additions');
db.exec('DELETE FROM menu_items');
db.exec('DELETE FROM categories');
db.exec('DELETE FROM admin_users');
db.exec('DELETE FROM settings');

db.prepare("INSERT INTO admin_users (username, password_hash, display_name) VALUES (?, ?, ?)").run(
  'admin',
  bcrypt.hashSync('zaitoun123', 10),
  'مدير زيتون'
);
...
for (const add of additions) {
  const r = db.prepare('INSERT INTO additions (name, price) VALUES (?, ?)').run(add.name, add.price);
  addIds[add.name] = r.lastInsertRowid;
}
...
const menuItems = [
  ...
  { name: 'شاي', category: 'مشروبات', description: 'شاي ساخن', base_price: 1.00, additions: ['سكر إضافي'] },
  { name: 'قرفة', category: 'مشروبات', description: 'مشروب قرفة ساخن', base_price: 1.50, additions: [] },
];

const insertItem = db.prepare('INSERT INTO menu_items (category_id, name, description, base_price, image) VALUES (?, ?, ?, ?, ?)');
const insertLink = db.prepare('INSERT INTO item_additions (item_id, addition_id) VALUES (?, ?)');

const transaction = db.transaction(() => {
  for (const item of menuItems) {
    const r = insertItem.run(catIds[item.category], item.name, item.description, item.base_price, null);
    const itemId = r.lastInsertRowid;
    for (const addName of item.additions) {
      if (addIds[addName]) {
        insertLink.run(itemId, addIds[addName]);
      }
    }
  }
});
